...
export const FORGE_CAPABILITIES = [
  ...
  },
  {
    ...
    description: 'Read the workspace selected by immutable workspace identity and return its version/head evidence.',
    inputSchema: EMPTY_INPUT,
    outputSchema: objectOutput({
      workspaceId: { type: 'string' },
      workspace: { type: 'object', additionalProperties: true },
      ...
      origin: { type: 'string' },
    }, ['workspaceId', 'workspace', 'version', 'workspaceHash', 'lastUpdated', 'origin']),
    context: { workspace: 'required', profile: 'none' },
    access: { public: false, studioSession: true, agentScopes: ['read', 'write', 'deploy'] },
    effects: ['read'],
    confirmation: 'none',
    ...
    surfaces: {
      ui: [
        connected('agent-bridge', 'src/components/AgentBridge.tsx::/api/agent/workspace'),
        connected('vscode-extension-workspace', 'vscode-extension/src/extension.ts::/api/agent/workspace'),
      ],
      cli: [],
      mcp: [partial('get_workspace', 'vscode-extension/mcp/x4forge-mcp.cjs::name: "get_workspace"', 'MCP returns a summary capped at 50 nodes rather than the full workspace/head.')],
      agentApi: true,
