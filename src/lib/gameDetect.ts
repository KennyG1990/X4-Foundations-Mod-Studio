...
export interface SetupProposal {
  x4GamePath: string;
  /** Installed/deployed extensions root for browse, import, and drift comparison. */
...
\t}
  ...
  ok('proposal_filesystem_is_deployed_extensions', p.filesystemPath === 'D:\\SteamLibrary\\steamapps\\common\\X4 Foundations\\extensions', p.filesystemPath);
  ok('proposal_workspace_under_documents', p.modWorkspacePath === 'C:\\Users\\example\\Documents\\X4ForgeMods', p.modWorkspacePath);
  ok('proposal_roles_are_distinct', p.filesystemPath !== p.modWorkspacePath, `${p.filesystemPath} == ${p.modWorkspacePath}`);
